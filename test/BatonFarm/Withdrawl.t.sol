...
contract WithdrawlTest is Fixture {
    ...
    function testWithdrawWithLPFee() public {
        vm.startPrank(batonMonitor);
        batonFactory.proposeNewLPFee(500); // 5%
        ...
        assertEq(initialStakeBal, 1 ether); // 1 ether in the farm
        assertEq(initialStakingTokenBal, 9 ether); // 1 ether staked
        assertEq(ethPairLpToken.balanceOf(batonMonitor), 0); // no fee collected yet
        ...
        assertEq(postStakeBal, 0); // no ether staked
        assertEq(postStakingTokenBal, 9_950_000_000_000_000_000); // retrieved
        assertEq(ethPairLpToken.balanceOf(batonMonitor), 50_000_000_000_000_000); // fee collected 5 percent
        vm.stopPrank();
    }
    ...
    function testWithdrawAndRemoveFromLp() public {
        vm.startPrank(owner);
        address batonFarmAddress =
            batonFactory.createFarmFromExistingPairETH{ value: 1 ether }(owner, address(ethPair), weekDuration);
        BatonFarm farm = BatonFarm(payable(batonFarmAddress));
    ...
    function testWithdrawAndRemoveFromLp_amountZero() public {
        vm.startPrank(owner);
        address batonFarmAddress =
            batonFactory.createFarmFromExistingPairETH{ value: 1 ether }(owner, address(ethPair), weekDuration);
        BatonFarm farm = BatonFarm(payable(batonFarmAddress));
    ...
    function testWithdrawAndRemoveFromLp_minBaseTokenOutputAmountTooHigh() public {
        vm.startPrank(owner);
        address batonFarmAddress =
            batonFactory.createFarmFromExistingPairETH{ value: 1 ether }(owner, address(ethPair), weekDuration);
        BatonFarm farm = BatonFarm(payable(batonFarmAddress));
    ...
    function testWithdrawAndRemoveFromLp_minFractionalTokenOutputAmountTooHigh() public {
        vm.startPrank(owner);
        address batonFarmAddress =
            batonFactory.createFarmFromExistingPairETH{ value: 1 ether }(owner, address(ethPair), weekDuration);
        BatonFarm farm = BatonFarm(payable(batonFarmAddress));
